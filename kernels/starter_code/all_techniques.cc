    ...
    for (int row = 0; row < m; row++) {
        for (int col = mat_args->start_j; col < mat_args->end_j; col++) {
            ...
            for (int q = 0; q < num_block; q += 4) {
                ...
                __m256i lower_w_next = _mm256_and_si256(raw_w_next, lowMask);
                __m256i higher_w_next = _mm256_srli_epi16(raw_w_next, 4);
                higher_w_next = _mm256_and_si256(higher_w_next, lowMask);
                ...
                w_0_next = _mm256_sub_epi8(lower_w_next, zero_point);
                w_128_next = _mm256_sub_epi8(higher_w_next, zero_point);
...
void MatmulOperator::mat_mul_all_techniques(struct matmul_params *params) {
    ...
    for (int i = 0; i < num_thread; ++i) {
        ...
        threads_args[i].end_j = std::min((i + 1) * cols_per_thread, n);
