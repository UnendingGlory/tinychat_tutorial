...
void MatmulOperator::mat_mul_simd_programming(struct matmul_params *params) {
    ...
    for (int row = 0; row < m; row++) {
        for (int col = 0; col < n; col++) {
            ...
            //            low|----------------------------------------------------------|high
            //               0                         256 bit
            __m256 acc0 = _mm256_setzero_ps(); // 256 bits can process 32 8-bits nums at an operation
            // pointer of the int4 weights
            ...
            const int num_block = k / block_size;
            // Compute two blocks in each iteration, each block 32 8-bits nums, 64 4-bits nums
            ...
            float *ptr = (float *)&acc0; // 256bits to 8 32-bits floats
